...
- **`reminder_policy.py`**: Category-specific reminder policies (`ExamReminderPolicy`, `LectureReminderPolicy`, `VideoMeetingReminderPolicy`, `TransitReminderPolicy`, `BillReminderPolicy`, `GeneralReminderPolicy`) managed via `ReminderPolicyRegistry` with adaptive presence suppression and recurring interval reminder cadence for unpaid bills.
...
- **`classifier.py`**: High-level event classification orchestrator providing `EventClassifier`, pre-compiled per-category regex registry (`_CategoryRegexRegistry`, `category_matches`), and backward-compatible re-exports. Delegates modular sub-concerns across dedicated domain components:
  - **`temporal_parser.py`**: Status marker stripping (`[CANCELLED]`, `Tentative:`), idiom overrides (`lunch and learn`), prefix detection (`study:`, `volo:`), and iterative regex-based temporal anchor stripping (`after dinner`, `on train`, `pre-workout`, `(was: old title)`).
  - **`category_theme.py`**: Data-driven presentation theming encapsulating `CategoryTheme` and the `_CATEGORY_THEMES` declarative mapping table (pilot selection, Catppuccin theme names, provider labels, action button text, Apple Maps deep link construction, and travel mode resolvers).
  - **`classification_rules.py`**: Deterministic rule-based pipeline (`ClassificationContext`, `ClassificationRule` base, and `CLASSIFICATION_PIPELINE`) executing sequential precedence rules: `CalendarMappingRule`, `VideoMeetingUrlRule`, `IdiomOverrideRule`, `EmptyCoreTitleFallbackRule`, `PrefixRule`, `StructuredFoodRule`, `FoodStartsWithRule`, `TravelRule`, `AcademicRule`, `SimpleCategoryKeywordRule`, `SpecialMissionPlatypusRule`, `QuickSyncSquirrelRule`, and `GenericPhysicalLocationRule`.
  - **`mascot_customizer.py`**: Dynamic mascot presentation and outfit customization authority (`MascotCustomizer`) mapping legacy pilot identifiers to animal/outfit combinations and category defaults.
  - **`keywords_data.py`**: Centralized bilingual dictionary datasets (`ENGLISH_KEYWORDS`, `ITALIAN_KEYWORDS`, and `build_default_keywords()`) for categories (`chef`, `captain`, `owl`, `gym`, `driver`, `zen_duck`, `work`, `concert`, `bill`, etc.).
